...
%CON_PTBPRS  Pool perturbation sessions: positions, perturbation parameters,
%   speed time series and timing, for one animal.
%   m = con_ptbprs(genpath) loads every session .mat in genpath and returns
%   target/response positions, perturbation amplitudes (linear v, angular w),
%   perturbation onset (nominal and detected), heading at perturbation onset,
%   displacement caused by the perturbation, per-trial speed traces and
%   indices for good / perturbed / forward / rewarded trials.
%
%   Relies on helper functions from the perturbation working folder
%   (add_ptbDisp, get_speedts, add_hindex2, add_theta_ptb, add_ptbn2); see
%   README > Status.
%   Used by split_trials, AUC_scatter, PCI_monkeys.
