...
%CONCAT_BASISF2  Pool perturbed trials with their continuous speed traces,
%   for the perturbation-kernel analysis.
%   [m, m2] = concat_basisF2(genpath) concatenates perturbed trials (events,
%   logical, prs, continuous) across sessions, together with the matching
%   unperturbed trials from con_ptbprs, so that get_kernel can regress the
%   response on the perturbation amplitude.
%
%   Relies on helper functions from the perturbation working folder
%   (add_theta_ptb, add_hindex1, add_ptbn2, add_continuous); see README > Status.
