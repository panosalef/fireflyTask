...
%GEN_SIM_PTB  Non-parametric simulation of "perturbed" trials from unperturbed ones.
%   [sim_resp, sim_targ] = gen_sim_ptb(r_np, th_np, v_np, w_np, r_p, th_p, t_ptb, v_ptb, w_ptb)
%   For every perturbed trial (target at r_p, th_p), picks one of the five
%   unperturbed trials with the closest target, adds the perturbation velocity
%   profile (Gaussian template, amplitudes v_ptb/w_ptb at time t_ptb) to that
%   trial's recorded linear/angular speed, and integrates to the simulated
%   stopping position. This is the "what if the animal ignored the
%   perturbation" control used for the ROC/AUC comparison.
