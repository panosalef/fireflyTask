...
%SPLIT_TRIALS  Split the pooled struct from con_ptbprs into trial classes.
%   ms = split_trials(m) returns perturbed (ms.p) and unperturbed (ms.np)
%   trials, each with target and response positions in polar and Cartesian
%   coordinates, speed time series and per-class session numbers, and the
%   perturbed set further split by whether the perturbation was helpful
%   (pushed toward the target) or not.
%%%%%%%%%%%%%%%%
%% Ptb Trials %%
%%%%%%%%%%%%%%%%
