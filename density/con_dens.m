...
%CON_DENS  Pool the optic-flow-density sessions of one animal into one struct.
%   m = con_dens(genpath) loads every session .mat in genpath (a `behaviours`
%   object each), concatenates final target and stopping positions, and splits
%   them by ground-plane density (m.keep.<level>.targ/resp in polar and
%   Cartesian coordinates). Densities are read from trials(j).prs.floordensity.
