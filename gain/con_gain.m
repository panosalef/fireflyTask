...
%CON_GAIN  Pool the joystick-gain sessions of one animal into one struct.
%   m = con_gain(genpath) loads every session .mat in genpath (each holding a
%   `behaviours` object from the firefly-monkey pipeline), concatenates
%   final target and stopping positions (polar), movement onset/offset,
%   reward and mean speed, and splits them by controller gain (1, 1.5, 2)
%   into m.split.g1 / g15 / g2 (and rewarded-only in m.rew.*).
%   Also returns travel time per trial (m.all.t_travel).
%
%   Used by time_cdf_fun, LOGregress_fun.
